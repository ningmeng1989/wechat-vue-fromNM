    ...
    <div class="_full_inner _scroll _effect component-chat" :class="{'_effect--30':decline}">
        ...
        <ul class="wechat-list">
            <li class="item _line-fine" v-for="item in wechat_list" transition="chat-item">
                <div class="info" :class="{'current':currentIndex==$index}" @touchstart="info_touchstart($index)" v-touch:tap="info_tap($index)" v-touch:swipeleft="info_swipeleft($index)" v-touch-options:swipe="{direction:'horizontal'}">
                    <div class="ico-box">
                ...
                <div class="handle">
                    <div class="handle-unread" v-touch:tap='increase_newsState($index,1)' v-show="item.chatBaseModel.newsUnreadCount==0">
                        标为未读
                    </div>
                    <div class="handle-unread" v-touch:tap='increase_newsState($index,0)' v-show="item.chatBaseModel.newsUnreadCount>0">
                        标为已读
                    </div>
...
export default{
        ...
        computed_unRead_count(){
            ...
            this.wechat_list.forEach(({
                ...
                if(!chatConfigModel.newsMute){
                    let count=chatBaseModel.newUnreadCount;
                    sum+=count;
                }
            });
    ...
    filters:{
        f_news(obj,attr){
            var obj=obj||{};
            let newsClass=obj.newsMute?'_news-dot':'_news-dount';
